...
const updateScoreBoard = () => {
    ...
    p1Ats.ariaLabel = `Player One has ${Game.getP1AllTime()} all time win.`;

    const cpAts = document.getElementById("cp_all_time_score");
    cpAts.textContent = Game.getCpAllTime();
    cpAts.ariaLabel = `Computer Player has ${Game.getCpAllTime()} all time win.`;
...
const finishGameFlow = (playerChoice) => {
    ...
    const actionMessage = builActionMessage(winner, playerChoice, computerChoice);
...
const builActionMessage = (winner, playerChoice, computerChoice) => {
